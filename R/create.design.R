...
create.design <- function(X, Z1, Z2, first.object, second.object, m, subject, control, order.Z1, order.Z2){
  ...
  if(!is.null(Z1)){
    ...
    for(pp in 1:p.Z1){
      ...
      design.help.Z1 <- create.design.X(design.help, Z1.help[subject,order.Z1[,pp]])
  ...
  if(!is.null(Z2)){
    p.Z2 <- ncol(Z2)/m
    for(pp in 1:p.Z2){
      Z2.help <- rowSums(design.help*(Z2[subject,((pp-1)*m)+(1:m)])[,order.Z2[,pp]])
  ...
  design <- cbind(design.order, design.help, design.X, design.Z1, design.Z2)
...
create.design.X <- function(design.help, X){
  design <- matrix(c(apply(X,2,function(xx){xx*c(design.help)})),nrow=nrow(design.help))
