...
check <- function(Z, object.names, subject){
  ...
  objects.Z <- matrix(word(colnames(Z),2,sep=fixed('.')),ncol=m,byrow=TRUE)
  
  ## number of Z-covariates
  p.Z <- nrow(objects.Z)
  
  ## order of the objects compared to general order from response
  order.Z <- apply(objects.Z,1,order)
  
  ## names of the Z-covariates
  var.Z <- matrix(word(colnames(Z),1,sep=fixed('.')),ncol=m,byrow=TRUE)
  

  ## 1. names of the objects have to be identical
  ## 2. within Z, objects have to be equal for all covariates, in the same order
  if(!all(apply(objects.Z,1,fun.check, object.names)) | 
       !all(apply(var.Z[,-1,drop=FALSE],2,function(x){identical(unique(c(x)),unique(c(var.Z[,1,drop=FALSE])))}))){
  text <- paste("The matrix", Z.name, "has to be arranged so that all columns 
                ...
                The colnames of", Z.name, "have to be named according to the scheme 
  ...
  ## every subject needs to have its individual line in the Z matrix, identified with rownames
...
####
## function to reparameterize from reference category to symmetric side constraints
####
reparam <- function(x){
  z <- ncol(x)+1
...
####
## function to create complete coefficient matrix
## either together with zero-columns for reference categories or with symmetric side constraints
####
expand.coefs <- function(coef, D, Y, symmetric = TRUE){
  ...
  if(n.order>0){
    order.effects <- coef[,(n.theta+1):(n.theta+n.order),drop=FALSE] 
    coef.new <- cbind(coef.new, order.effects)
  }
  
  ## if intercepts parameters exist, reparameterize them
  if(n.intercepts>0){
    intercepts <- coef[,(n.theta+n.order+1):(n.theta+n.order+n.intercepts),drop=FALSE]
    if(symmetric){
  ...
  ## if parameters for subject-specific covariates exist, reparameterize them
  if(p.X > 0){
    gamma <-  coef[,(n.theta+n.intercepts+n.order+1):(n.theta+n.intercepts+n.order+p.X*(m-1)),drop=FALSE]
    index <- 1
    for(i in 1:p.X){
      if(symmetric){
        coef.X <- reparam(gamma[,index:(index+m-2),drop=FALSE])
      }else{
        coef.X <- cbind(0,gamma[,index:(index+m-2),drop=FALSE])
      }
      colnames(coef.X) <- paste(rep(vars.X[i], each = m), object.names,sep=".")
      coef.new <- cbind(coef.new, coef.X)
      index <- index+m-1 
    }
  }

  ## if parameters for object-specific covariates exist, leave them as they are
  if(p.Z1+p.Z2 > 0){
    rest <- coef[,(n.theta+n.intercepts+n.order+p.X*(m-1)+1):ncol(coef),drop=FALSE]
    if(p.Z1>0){
    colnames(rest) <- c(paste(rep(D$vars.Z1, each = m), object.names,sep="."),
                        D$vars.Z2)
    }else{
